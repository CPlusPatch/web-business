...
const update = (name: string, value: UISetting["value"]) => {
	const updatedCategory = props.category.map(s =>
		s.name === name
			? {
					...s,
					value,
...
<template>
	<div v-for="setting in category" :key="setting.name">
		...
		<div v-if="setting.type === UISettingType.Image" class="col-span-full">
			...
			>
			<div
				class="mt-2 ring-1 ring-gray-200 flex items-center justify-center gap-x-3 relative h-15 w-15 rounded overflow-hidden group">
				<input
					accept="image/*"
					...
					class="hidden" />
				<div
					@click="!isUploading && clickFileInput(setting)"
		...
		<div v-if="setting.type === UISettingType.Toggle">
			<div class="flex flex-row items-center gap-3">
				<div
					@click="update(setting.name, !setting.value)"
					:class="[
						!!setting.value ? 'bg-orange-600' : 'bg-gray-200',
						'relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none outline-none',
					]">
					<span
						aria-hidden="true"
						:class="[
				...
				<label
					:for="setting.name"
					class="inline text-sm leading-6 text-gray-900 font-semibold"
					>{{ setting.title }}</label
				>
			</div>
			<div class="text-xs mt-1 text-gray-500">{{ setting.text }}</div>
