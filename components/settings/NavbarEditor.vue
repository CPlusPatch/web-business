...
<template>
	<div v-for="item in value">
		<label class="block text-sm leading-6 text-gray-900 font-semibold">Navbar Element</label>
		<Input :loading="isLoading" name="" :value="item.title" @input="updateValue(item.id, {
			title: ($event.target as HTMLInputElement).value
		})" icon="" placeholder="Link title" class="mt-2 w-full md:!w-96" />
		<Input :value="item.href" :loading="isLoading" name="" icon="" placeholder="Relative URL (/something)" @input="updateValue(item.id, {
