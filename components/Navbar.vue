...
const debounce = <T extends any[]>(func: DebounceFunc<T>, delay: number): DebounceFunc<T> => {
...
<template>
	<header
		:class="[
			...
			shrunk ? 'bg-white/30 px-5 py-4' : 'bg-transparent p-8',
		]">
		<div
			:class="[
			...
			<Button @click="open = !open" class="!border-none !p-0 !outline-none !shadow-none md:hidden">
		...
		<Transition appear enter-from-class="opacity-0 -translate-y-4 sm:translate-y-0 sm:scale-95"
