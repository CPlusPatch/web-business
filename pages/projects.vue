...
const projects = [
	{
		...
		description: "Fedibase is my flagship project, a fast, beautiful and easy to use Fediverse client that gets things right. It is written in Vue.js and is the most popular of my applications",
		tags: [
			{
				name: "VueJS",
				color: "bg-green-100",
	...
	},
	{
		...
		description: "My new personal website, rewritten from the ground up using Nuxt.js and TailwindCSS: it is the 9th iteration of this particular project, preceded by CPlusPatch CMS Next",
		tags: [
			{
				name: "Nuxt.js",
	...
	{
		name: "CPlusPatch CMS Next",
		href: "https://codeberg.org/CPlusPatch/cpluspatch-cms-next",
		description:
			"My old personal website, part of CPlusPatch CMS. It is a modern application written from the ground up with Next.js and TailwindCSS, incorporated into my CMS.",
		tags: [
			{
				name: "React",
				color: "bg-sky-100",
...
<template>
	<PrimaryContainer class="flex flex-col gap-y-10">
		<h1 class="mb-3 text-3xl font-black text-gray-800 font-poppins sm:text-5xl">Projects</h1>
		<div>
			<h2 class="text-xs font-medium tracking-wide text-gray-500 uppercase">
				Pinned Projects
			</h2>
			<ul role="list" class="grid grid-cols-1 gap-5 mt-3 sm:gap-6 sm:grid-cols-2 lg:grid-cols-3">

				<li v-for="project of projects" :key="project.name" class="flex col-span-1 rounded-md shadow-sm font-inter duration-200 ease-in-out hover:shadow-xl hover:ring-2 ring-orange-400">
					<a :href="project.href"
						class="flex flex-col justify-between items-start p-3 max-w-full bg-white rounded-md border border-gray-200">
						<h3 class="pb-4 text-lg font-bold text-gray-900 hover:text-gray-600">
							{{ project.name }}
						</h3>
						<p class="text-gray-500">{{ project.description }}</p>
						<div class="flex flex-wrap flex-row gap-2 mt-3 max-w-full no-scroll">
							<span v-for="tag of project.tags" :key="tag.name"
								:class="`${tag.color} inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${tag.textColor}`">
								{{ tag.name }}
							</span>
						</div>
					</a>
